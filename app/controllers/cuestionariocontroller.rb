class CuestionarioController < ApplicationController
  ...
  post '/newcuestionario' do
    ...
    @cuestionario = Cuestionario.new(titulo: @data[0], descripcion: @data[1], creador: @user.name, npreguntas: array.length, notaaprobar: (array.length / 2.0), notamaxima: array.length, fechacre: Time.new)
